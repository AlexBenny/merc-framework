...
{
    public class Dispatcher : IMDispatcher, IFDispatcher
    {
        ...
        private IMerc merc;
        private MCDictionary<Dictionary<string,MethodInfo>> bindingMap;  //MessageContent <-> (State <-> Method)
            ...
            try
            {
                ...
                foreach (Attribute a in attrs)
                {
                    ...
                    if (tuple.match(filter))
                    {
                        ...
                        {
                            logInfo("Parameter " + (i + 1) + " mismatch for method " + mi.Name);
                            throw knfe;
                        }
                        // method invocation
